...
simtime     = 10*350.0   # simulation time [ms]
order       = 5500  # determines size of network:
...
raw = []
old = None
for i in np.arange(simtime/10.0,simtime,simtime/10):
    print(i)
    # run, measure computer time
    timer.start()  # start timer on construction
    print("%d Running simulation for %g ms." % (rank, simtime/10.0))
    run(float(simtime/10.0))
    simCPUTime = timer.elapsedTime()
    trains = BinnedSpikeTrain(E_net.get_data().segments[0].spiketrains, binsize=5*ms)
    times.append(trains)
    cov = elephant.spike_train_correlation.covariance(trains)
    ccf = elephant.spike_train_correlation.corrcoef(trains)

    raw.append(E_net.get_data().segments[0].spiketrains)

    try:
        for i,_ in enumerate(old):
            print(old[i]==raw[-1][0][i])
...
efull_trains = BinnedSpikeTrain(E_net.get_data().segments[0].spiketrains, binsize=5*ms)
...
ifull_trains = BinnedSpikeTrain(E_net.get_data().segments[0].spiketrains, binsize=5*ms)
...
def write_data(data,fname):
    with open(fname+str('.p'),'wb') as f:
        pickle.dump(data,f)
    return
data = [efull_trains,ifull_trains,icov,iccf]
_ = write_data(data,"Results/brunel_np%d_%s.pkl" % (np, simulator_name))
